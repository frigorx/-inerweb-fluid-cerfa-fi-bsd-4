...
/* ------------------------------------------------------------------
   ...
   ------------------------------------------------------------------ */
const RESERVE = {
  1: [{ ref: 'svg:consignation-cinq-etapes', legende: 'À dérouler avant de toucher au circuit — l’ordre ne s’inverse pas.' }],
  2: [{ ref: 'svg:classe-lettre-chiffre', legende: 'La lettre d’abord, le chiffre ensuite — deux dangers distincts.' }],
  3: [{ ref: 'svg:deux-etages-deux-papiers', legende: 'Du règlement européen à vos deux attestations.' }],
  4: [{ ref: 'svg:categories-champs', legende: 'Repérez votre colonne : elle dit ce que votre attestation couvre.' }],
  5: [{ ref: 'svg:croix-frigoriste-etats', legende: 'La croix se lit organe par organe — l’état du fluide à chaque coin.' }],
  6: [{ ref: 'svg:logph-lecture', legende: 'Le cycle se lit sur le diagramme, jamais de mémoire.' }],
  7: [{ ref: 'svg:mesures-surchauffe-sous-refroidissement', legende: 'Deux écarts qui se mesurent — jamais ne se devinent.' }],
  8: [{ ref: 'svg:code-nomme-classe-previent', legende: 'Cinq étiquettes, un réflexe : la classe se vérifie sur la FDS.' }],
  9: [{ ref: 'svg:compresseurs-comparatif', legende: 'Quatre technologies sur le terrain, la fonction ne change pas.' }],
  10: [{ ref: 'svg:condenseur-ecart-encrassement', legende: 'Le même condenseur, propre puis encrassé : l’écart raconte tout.' }],
  11: [{ ref: 'svg:givre-isole-machine-force', legende: 'Une batterie sous givre n’échange plus — la machine paie.' }],
  12: [{ ref: 'svg:ligne-liquide-protection', legende: 'Du réservoir au détendeur, chaque organe veille sur le suivant.' }],
  13: [{ ref: 'svg:sequence-mise-en-service', legende: 'L’ordre des opérations avant la première charge.' }],
  14: [{ ref: 'svg:recherche-fuite-geste', legende: 'Le geste du contrôle : lent, près du raccord, confirmé.' }],
  15: [{ ref: 'svg:recuperation-securisee', legende: 'Rien ne part à l’air : le fluide rejoint la bouteille, pesé.' }],
  16: [{ ref: 'svg:brasage-balayage-azote', legende: 'L’azote balaie l’intérieur du tube pendant toute la chauffe.' }],
  17: [{ ref: 'svg:quatre-leviers-energie', legende: 'Quatre choix de conception qui se voient sur la facture.' }],
  18: [{ ref: 'svg:r290-zone-intervention', legende: 'Le propane impose sa zone : balisée, ventilée, sans flamme.' }],
  19: [{ ref: 'svg:co2-nh3-deux-risques', legende: 'Deux fluides naturels, deux dangers — un même réflexe : s’arrêter.' }],
};
