...
const INTERIEUR = `${F.NOM_INTERIEUR}.pdf`;
const COUVERTURE = `${F.NOM_COUVERTURE}.pdf`;
...
/* Le dossier est VIDÉ avant d'être rempli. Sans cela, un changement de
   format y laisse l'ancien paquet à côté du nouveau — le 29/08/2026,
   `dist/kdp/` a contenu un 6 × 9 périmé et un 7 × 10 courant côte à côte,
   et rien ne disait lequel téléverser. Un dossier de téléversement ne
   contient QUE ce qu'on téléverse. */
fs.rmSync(KDP_DIR, { recursive: true, force: true });
...
|---|---|---|
| Format | **${F.FORMAT_LISIBLE}** (${F.COTES_LISIBLES}) | le fichier est à cette taille exacte, sur ses ${K.pages} pages |
