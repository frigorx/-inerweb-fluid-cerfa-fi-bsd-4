...
// DÉPENDANCES : aucune (DOM / réseau / store INTERDITS). Recopié EN LITTÉRAL
//   dans server/coffre-identites.js — parité prouvée par
...
export const LONGUEUR_MIN_PHRASE_COFFRE = 14;
 ...
 * @returns {string}
 */
export function aadIdentite(personnelId, pseudonyme) {
 ...
 */
export function libellePseudonyme(annee, numero) {
 ...
 * @returns {boolean}
 */
export function estFicheEchue(personne, dateDuJour) { // eslint-disable-line no-unused-vars
 ...
 */
export function assemblerIdentite(personne, annexes) {
  const a = annexes || {};
  return {
 ...
 */
export function pseudonymiserFiche(annee, numero) {
 ...
 * @returns {object} patch à appliquer à la fiche
 */
export function restaurerIdentite(identite) {
  return {
 ...
 * @returns {string|null}
 */
export function libelleIntervenant(champFige, personneId, indexPersonnel) {
