...
export const METHODES_CONTRAT = {
  ...
  signerMouvement: { genre: 'mutation',
    description: 'Pose une signature RÉELLE sur un mouvement BROUILLON : (mouvementId, { role TECHNICIEN|DETENTEUR, nom, prenom, qualite?, organisation?, parDelegation?, imagePng base64 }). Personne physique obligatoire (nom + prénom) ; DETENTEUR seulement après une signature TECHNICIEN de la même révision ; parDelegation exige la raison sociale représentée ; la déclaration est composée par le store (signatures-mouvement.js), jamais reçue du client ; image PNG RÉELLEMENT DÉCODÉE (lot B3, 25/07 : IHDR, chaîne des chunks, CRC-32 de chacun, IDAT, IEND, rien après — reconnaître 8 octets magiques ne prouvait rien ; plafond de 1 Mo contrôlé AVANT décodage ; AUCUNE borne basse — celle de 1 Ko est RETIRÉE, elle ne séparait rien) et REFUSÉE si la zone est restée vierge (image rigoureusement uniforme, MSG_ZONE_VIERGE) : aucun seuil d’encre, une griffure de deux pixels passe, et sur un format que l’on ne sait pas relire on ne conclut JAMAIS au vide. Une signature illisible n’est jamais ignorée : elle est refusée ; enregistre l’horodatage réel, l’empreinte du document et la révision signée — toute modification ultérieure du brouillon rend la signature PÉRIMÉE (par comparaison, jamais de retouche ni de suppression) ; MSG_ECRITURE_FIGEE si figé.' },
  getSignaturesMouvement: { genre: 'lecture',
    description: 'Les signatures réelles d’un mouvement (id) : [{ id, mouvementId, role, nom, prenom, qualite, organisation, parDelegation, dateHeure ISO, declaration, imagePng, sessionCompteId, sessionPersonnelId, sha256Document, versionDocument, valide, imageRecevable }] — valide = versionDocument égale la révision courante du brouillon ET image recevable (lot B3, revue du 25/07 : une image illisible entrée par un import ne vaut PAS signature — les conditions 14/15 du mode Officiel restent alors opposées) ; imageRecevable dit la CAUSE à part (revue du 26/07) : repliée dans valide, elle ressortait partout en « périmée », c’est-à-dire « la fiche a été modifiée après la signature », motif FAUX qui entrait jusque dans la colonne État de signatures.csv, au dossier scellé — l’écran et le CSV ont désormais un troisième état « image illisible » ; tri dateHeure puis id (comparaison de chaînes simple) ; copies indépendantes ; Error si mouvement introuvable.' },
