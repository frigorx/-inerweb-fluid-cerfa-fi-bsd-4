...
function dateRevueValide(v) {
  return typeof v === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(v)
    && Number.isFinite(Date.parse(v)) && new Date(v).toISOString().slice(0, 10) === v;
}

function preparerRevueFormation(mouvements, avant, jour, pieces = [], signatures = [], controles = []) {
...
function verifierSelectionFormation(revue, ids, confirmation, motif, copiesExternes) {
...
module.exports = { dateRevueValide, preparerRevueFormation, verifierSelectionFormation };
