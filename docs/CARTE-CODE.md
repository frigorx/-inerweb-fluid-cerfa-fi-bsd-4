...
| `licence.js` | licence d'évaluation NOMINATIVE (14/08, plan `docs/PLAN-LICENCE-NOMINATIVE.md`) : `licence-inerweb.json` signé Ed25519 vérifié 100 % en local (clé publique n° 1 embarquée, rotation ADDITIVE ; la privée vit chez le propriétaire, HORS dépôt) ; exigée si `node\node.exe` est à côté du serveur (signature de fabrication du paquet) ou `IWF_LICENCE_REQUISE=1` ; décision PURE `evaluerDemarrageLicence` appliquée par serveur.js — absente/falsifiée = refus + contact, **expirée = LECTURE SEULE** (mutations du contrat en 403, consultations/exports/sauvegardes ouverts à vie) | le dépôt de dev, le filet et la démo en ligne sont hors champ (pas de `node\`) ; ne rend PAS la copie impossible (code lisible, doctrine 14/07) — il rend le partage nominatif et traçable ; `test-licence` (paire jetable, la clé privée réelle ne touche jamais le filet) |
