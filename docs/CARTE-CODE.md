...
> 14/08/2026). Ce n'est pas un journal : aucune valeur périmée ne doit y rester.
...
- **Tests** : `node outils/lancer-tests.mjs --tout` = TOUT le filet
  (**140 exécutions, ~130 s** au 14/08/2026 ; sans `--tout`, arrêt au
...
| `coffre-identites.js` | miroir littéral des règles pures du COFFRE DES IDENTITÉS (lot E2) ; **lot C carte blanche (13/08) : `getMouvements` des DEUX stores rend le champ `technicien` par la fiche VIVANTE (pseudonyme) quand le porteur est au coffre — la 3e porte, tirée par la 4e relecture ; la donnée scellée ne bouge pas, et l'export JSON lit `lireMouvementsBruts` (le transport rejoue les empreintes)** | parité prouvée par `test-coffre-identites.mjs` ; api.js porte les 6 gestes (REFERENT/ADMIN + poste local), le témoin GCM, l'archive préalable OBLIGATOIRE, la purge rattrapée au démarrage (`rejouerPurgeCoffre`), les verrous de fiche au coffre (updatePersonne/PJ/désactivation/habilitations/mentions) et le transport export/import du coffre (E2c : coffreConfig sel/témoin/kdf + compteurs + enveloppes base64, remplacement atomique, simulation rejetée, refus protecteur si fichier sans coffre sur poste au coffre) ; primitives crypto = `chiffrement.js` (enveloppes de champ autoportantes, scrypt N=131072) |
...
| `comptes.js` / `sessions.js` / `routes-comptes.js` | scrypt **N=2^17** (OWASP, P2-3)+NFC+leurre anti-timing, jetons hachés SHA-256, cookie HttpOnly ; **lot A carte blanche (13/08) : connexion ASYNCHRONE (A14) — `crypto.scrypt` en pool de threads derrière une file à UNE dérivation, bornée à 64 (au-delà 503, jamais un gel) ; verrou EXPIRÉ = fenêtre NOUVELLE de 5 essais (le compteur repart à 1, sinon re-verrou perpétuel — 4e relecture, tiré)** | message d'échec UNIQUE **sur TOUS les refus, VERROU COMPRIS (l'ancien « Compte verrouillé » 403 ne sortait que pour un login existant : énumération) — l'état de verrou se lit dans listerComptes (ADMIN)** ; session meurt si compte désactivé ; ancien profil N=2^15 accepté puis RE-HACHÉ à la connexion (journal `RENFORCEMENT_HASH_MOT_DE_PASSE`) ; chiffrement.js garde N=2^15 (archives existantes) |
| `sauvegarde.js` / `restauration.js` / `manifeste.js` / `verification.js` / `chiffrement.js` | coffre-fort : VACUUM INTO, restauration atomique, AES-256-GCM ; **lot 0/B3 (27/07) : `validerDossierDestination` REFUSE une destination sous OneDrive/Drive/Dropbox (`db.cheminSousSynchronisation`, dérogation `IWF_AUTORISER_SAUVEGARDE_SYNCHRONISEE=1`) — la base vive y était déjà interdite (P1-6) pendant que les archives AUTOMATIQUES, en clair et NOMINATIVES, y allaient sans un mot. Garde dans le SERVEUR donc valable API comprise ; refus AVANT tout `mkdir` ; un réglage DÉJÀ enregistré n'empêche PAS de démarrer (`avertissementDestinationSynchronisee`, affiché par `serveur.js`) — bloquer aurait arrêté les sauvegardes d'un poste. `test-destination-synchronisee`** | jamais copier le .db à chaud ; phrase NFC ; rollback = reposer l'original ; ⚠️ verification.js hache les mouvements (4e vérificateur versionné v2 — l'oublier rendrait toute archive « invalide ») |
...
| `borne-scellement.js` | **« ce poste a déjà scellé »** (L2, 25/07) — borne MONOTONE du nombre d'écritures scellées, qui ne DESCEND jamais ; c'est elle qui refuse le ré-amorçage de chaîne à l'import (« historique antérieur au scellement » forgé en retirant toutes les empreintes) | ⚠️ elle vit dans un fichier VOISIN de la base, PAS dedans : première version en table `parametres`, mise en défaut par la revue en la TIRANT (la restauration d'archive remplace la base, donc la borne). Une protection contre l'écrasement du registre ne peut pas vivre DANS le registre. Ne prétend rien contre qui a le disque : elle ferme le canal IMPORT/RESTAURATION. **Lot 0 / B2 (27/07) : elle est enfin CONFRONTÉE au registre AU DÉMARRAGE** — `api.constaterBorneScellement()` (api.js, juste après `nombreScelleesJamaisAtteint`) compare la borne au COUNT des écritures portant une empreinte et rend `{ok, motif, borne, reelles}` ; `serveur.js` l'appelle AVANT l'écriture du témoin quotidien, **best-effort absolu** (jamais fatal, jamais bloquant : un registre qu'on ne peut plus ouvrir serait pire que le défaut), console explicite + entrée au journal chaîné `REGISTRE_REGRESSION`. ⚠️ La mesure compare DEUX NOMBRES : recopier la base ET son fichier voisin ensemble rend l'écart nul (limite CONNUE), donc on ne dit JAMAIS « le registre est intact », mais « la borne et le registre concordent / ne concordent pas ». Borne absente ou illisible = **INDÉTERMINÉ**, jamais une accusation (doctrine png.js). ⭐ **Revue B2 : « RÉGRESSION » NE DÉSIGNE PAS UN COUPABLE** — tiré en le jouant (suite, section 5), une RESTAURATION d'archive plus ancienne (geste PRÉVU, confirmé par `confirmePerte`) produit le MÊME écart, puisqu'elle remplace la base et pas le fichier voisin ; un IMPORT d'un registre plus court aussi. Le message imputait « une base remise en place à la main », c'est-à-dire une cause NON mesurée, à chaque démarrage et pour toujours : il énumère désormais les trois causes possibles et DIT qu'il ne tranche pas. ⚠️ Conséquence assumée : après une restauration légitime, le constat reste RÉGRESSION à chaque démarrage (la borne ne redescend jamais — c'est sa définition). `test-non-regression-scellement` |
| `licence.js` | licence d'évaluation NOMINATIVE (14/08, plan `docs/PLAN-LICENCE-NOMINATIVE.md`) : `licence-inerweb.json` signé Ed25519 vérifié 100 % en local (clé publique n° 1 embarquée — ROTATION du 14/08 après lecture de la 1re clé par un outil d'audit tiers ; rotation ADDITIVE, la privée vit chez le propriétaire HORS dépôt, ACL resserrées) ; exigée si `node\node.exe` est à côté du serveur ou `IWF_LICENCE_REQUISE=1` ; décision PURE `evaluerDemarrageLicence` appliquée par serveur.js — absente/falsifiée = refus + contact, **expirée = LECTURE SEULE : UNE règle en AMONT de TOUS les aiguillages** (revue externe 14/08 — la garde d'origine vivait après les routeurs, bootstrapAdmin et definirCodeExercice écrivaient encore ; classification COMPLÈTE des routes hors contrat ICI : fermées = création d'identités + code d'exercice, ouvertes = connexion, gouvernance d'accès, coffre-fort entier — prouvée exhaustive par test-licence contre les `METHODES` des 3 routeurs) | clé n° 0 « d'essai » honorée UNIQUEMENT hors paquet (banc du filet ; dans un paquet elle n'ouvre rien) ; `IWF_LICENCE_FICHIER` = chemin alternatif sans droit ; ne rend PAS la copie impossible (doctrine 14/07) ; `test-licence` **50 vérifs** dont serveur RÉEL expiré joué |
| `version.js` | SOURCE UNIQUE des versions (revue 14/08) : `VERSION_LOGICIEL` (recopiée par package.json, concordance exigée à la fabrication) + `NODE_VALIDE` (version EXACTE de Node embarquée dans les paquets — la fabrication refuse toute autre) | contrat et schéma ont LEURS sources (contrat.js, migrations.js) — ne pas les recopier ici |
| `creer-admin.js` | CLI bootstrap 1er ADMIN | aucun endpoint web équivalent |
| `secours-compte.js` | CLI porte de secours (lot A carte blanche, 13/08) : déverrouiller un compte / remplacer son mot de passe depuis le POSTE (autorité = accès physique, comme creer-admin) | gestes journalisés (`DEVERROUILLAGE_COMPTE`, `REINIT_MOT_DE_PASSE`) ; remplace l'édition manuelle du fichier SQLite ; aucun endpoint web équivalent |
| `routes-exercice.js` | MODE EXERCICE (13/08, plan `docs/PLAN-MODE-EXERCICE.md`) : code de déblocage (défini ADMIN/REFERENT, haché scrypt async) + `demarrerExercice` = la PHOTO du registre (export complet) contre session + code | « code faux » et « code non défini » indiscernables ; chaque tirage JOURNALISÉ (`DEMARRAGE_EXERCICE`) ; le code n'apparaît jamais en clair ; hors contrat DataStore (patron routes-sauvegarde) |
...
| `demo-store.js` (6 632 l. au 14/08) | implémentation mémoire complète (référence sémantique) |
...
| `datastore.js` | fabrique : choisit DemoStore ou LocalStore selon que le serveur répond ; **MODE EXERCICE (13/08) : le drapeau posé force le BAC À SABLE (DemoStore) même quand le serveur répond, semé de la PHOTO du réel par `importerJSON` au premier chargement** |
| `mode-exercice.js` | cycle de vie du BAC À SABLE pédagogique (13/08, plan `docs/PLAN-MODE-EXERCICE.md`) : drapeau, photo d'origine + date, semis, réinitialisation, **effacement TOTAL d'un geste** (« toute trace a été détruite » — stockage vide, mesuré) | stockage INJECTABLE (testable pur, silencieux sans localStorage) ; l'exercice persiste entre les sessions jusqu'à l'effacement ; s'exporte/se recharge en fichier (contrat) ; UI = `views/bandeau-exercice.js` (3 gestes, jamais imprimé) + carte de l'écran Sauvegarde |
...
| `blocage-officiel.js` | pur : moteur de blocage du mode OFFICIEL (lot B) — `evaluerBlocagesOfficiel(cadre)` applique la liste de `docs/CONDITIONS-BLOCANTES-OFFICIEL.md` filtrée par moment (PASSAGE/SOUMISSION/VALIDATION), `VERROU_LIVRAISON` ferme le mode jusqu'aux lots C-D ; branché aux 3 moments des deux stores + `simulerValidationOfficielle` (contrat) ; **lot F carte blanche (13/08) : condition 19 `CAPACITE_ETABLISSEMENT` — la PORTÉE de l'attestation de l'établissement (fait `capaciteEtablissement` des deux cadres) doit couvrir l'intervention, `habilitations.capaciteEtablissementCouvre` délègue à la MÊME matrice que l'aptitude (aucune grille nouvelle), portée vide = rien d'autorisé ; suite doublée `test-capacite-etablissement`** ; conditions 14-15 (lot C) = faits tri-état signatureTechnicienValide/signatureDetenteurValide ; P7-c : `MSG_CONTROLE_DIRECT_OFFICIEL` = refus STRUCTUREL de `createControle` en OFFICIEL (FORMATION-only par nature, l'officiel = mouvement type CONTROLE) |
...
| `equipement.js` | pur : MODÈLE D'ÉQUIPEMENT (P1-1) — `detectionEffective` (E1 : allègement de fréquence dû seulement si détection vérifiée < 12 mois ; 4 motifs), `detectionObligatoire` (E2 : vrai au niveau HAUT — **interroge `evaluerControle`, aucun seuil recopié**), `exemptionControle` (**E3(b)/L5 24/07 : le calcul est CODÉ (`calculerExemption`, 3 seuils stricts Q6, cas R2 chiffré) derrière `EXEMPTION_HERMETIQUE_ACTIVE = false` — fermé tant que les trois conditions qui remplacent le visa T3 (abandonné le 26/07/2026, docs/NOTE-DECISION-ETABLISSEMENT.md §4) ne sont pas réunies, geste d'activation consigné au PLAN-LOTS §L5**), `hermetiqueOpposable` (E4 : seuil d'aptitude 6 kg si scellé ET **étiqueté**), `mobileListe` (E5 : liste FERMÉE `SOUS_TYPES_MOBILES_ELIGIBLES`, AUTRE_MOBILE exclu), `verifierModeleEquipement` (garde de saisie), `echeanceVerificationDetection` (+12 mois civils). **L2 (25/07) : la date de vérification exige le calendrier RÉEL (regex enfin ANCRÉE) et une date FUTURE ne vaut plus vérification — défense en profondeur dans `detectionEffective`, pour le cas où la valeur entre par l'import.** Miroir littéral CommonJS `server/equipement.js`, parité prouvée par `test-equipement-pur` ; comportement métier par `test-equipement` (doublée). Consommé par : le moteur de fréquence des 2 stores (détection EFFECTIVE), `getAlertes` (alr-detection-*), les 2 `cadreFicheOfficiel` (faits `intervenant.aptitude` P0-5 + `detectionObligatoireAbsente` cond. 17), `dossiers-fuite.estMachineMobile`, `machine-form`, `fiche-machine` |
...
- `wizard/` : les 6 étapes du mouvement (`wizard.js`, 2 203 l. au 14/08) + signature canvas
