...
3. **P0-7, en briques, Opus effort xhigh, petites étapes :**
   ...
   - **Écarts consignés (à trancher avec Franck, non bloquants)** : (a) l'annulation d'un mouvement CONTROLE ne neutralise pas les effets machine du contrôle lié (dernierControle/échéance/statut restent) — même comportement que le contrôle accessoire annulé, préexistant ; à revoir avec P0-6 (cycle fuite) si besoin ; (b) l'échéance du contrôle ACCESSOIRE (charge+contrôle déclaré) n'est pas mise à jour (historique, cf. P7-d1).
