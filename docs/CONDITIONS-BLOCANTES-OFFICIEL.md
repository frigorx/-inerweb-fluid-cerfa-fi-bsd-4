...
| 16 | APTITUDE OPPOSABLE : l'habilitation de l'intervenant COUVRE cette intervention — matrice catégorie × opération × famille de fluide × charge NOMINALE de la machine (moteur `verifierDroitIntervention`, fait `intervenant.aptitude` précalculé par les stores) ; une attestation 2008 est reconnue jusqu'au 12/03/2029 puis SEULEMENT avec une remise à niveau enregistrée au plus tard le butoir, cycle 7 ans (`habilitationReconnue` L4/Q3 24/07, appliquée aussi à la n° 7 — l'ancien couperet du 31/12/2026 confondait délivrance et validité) ; jamais posée en doublon de la n° 7 ; une réserve non bloquante du moteur (gravité CONSEIL) ne bloque JAMAIS *(P0-5, 22/07 — code `APTITUDE_PORTEE`)* | S·V |
...
| 19 | PORTÉE DE LA CAPACITÉ DE L'ÉTABLISSEMENT : l'attestation déclarée COUVRE cette intervention — les `categoriesAutorisees` entrent dans la MÊME matrice que l'aptitude de la personne (`capaciteEtablissementCouvre`, régime déduit de chaque catégorie : I-IV → 2008, A1…V → 2025), et l'activité réglementée requise par le type d'intervention doit figurer dans `activitesAutorisees` (INSTALLATION → MISE_EN_SERVICE, MAINTENANCE → MAINTENANCE, ETANCHEITE → CONTROLE, RECUPERATION → RECUPERATION). Une portée VIDE n'autorise RIEN (le doute retire l'allègement) ; fait `capaciteEtablissement` précalculé par les stores ; jamais posée en doublon des n° 1-4 : sans attestation déclarée, elles seules parlent. ⚠️ Blocage n° 1 de la 4e relecture externe (27/07), TIRÉ : avant ce lot la portée était saisie, validée en forme, stockée, affichée — et lue par AUCUNE règle ; une récupération de 8 kg sur 50 kg passait sur un établissement « catégorie II, contrôle seul » *(lot F carte blanche, 13/08 — code `CAPACITE_ETABLISSEMENT`)* | S·V |
